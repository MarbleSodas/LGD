...
const ENABLE_ARGUMENT := "--enable-godot-cli"
...
func _ready() -> void:
	...
	var err := _server.listen(port, LOOPBACK_ADDRESS)
	if err != OK:
		push_error("GodotCLI: Failed to listen on port %d: %s" % [port, error_string(err)])
		return
	print("GodotCLI: Server listening on %s:%d" % [LOOPBACK_ADDRESS, port])
...
func _resolve_allowed_file_path(path: String) -> String:
	...
	if _is_path_within_root(resolved_path, project_root):
		return resolved_path
	if _is_path_within_root(resolved_path, user_root):
		return resolved_path
	return ""


func _is_path_within_root(path: String, root: String) -> bool:
	var root_prefix: String = root if root.ends_with("/") else root + "/"
	return path == root or path.begins_with(root_prefix)
...
func _cmd_create_file(params: Dictionary) -> Dictionary:
	...
	var abs_path: String = _resolve_allowed_file_path(path)
	if abs_path.is_empty():
		return {"status": "error", "error": "Path must stay within res:// or user://"}
